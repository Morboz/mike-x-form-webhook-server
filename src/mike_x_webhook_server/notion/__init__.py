...
class NotionClient:
    ...
    def create_page_in_database(self, database_id: str, page_title: str, properties: dict) -> str:
        """在数据库中创建新的页面（行记录）
        ...
        try:
            ...
            logger.error(f"Failed to create page '{page_title}' in database {database_id}: {str(e)}")
            raise NotionAPIError(f"Failed to create page: {str(e)}")

    def create_database_in_page(self, page_id: str, title_plain_text: str, properties: list[str]) -> str:
        """在指定页面下创建数据库
    ...
    def get_database_by_title_text(self, page_id: str, title_plain_text: str) -> Optional[Dict]:
        """根据标题文本查找数据库
        ...
        try:
            ...
            logger.error(f"Failed to find database with title '{title_plain_text}': {str(e)}")
